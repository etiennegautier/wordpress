...
<?php 
add_action( 'the_retailer_woocommerce_before_checkout_form', 'woocommerce_checkout_coupon_form', 10 );
do_action('the_retailer_woocommerce_before_checkout_form');
?>
...
if (get_option('woocommerce_enable_signup_and_login_from_checkout')=="no" && get_option('woocommerce_enable_guest_checkout')=="no" && !is_user_logged_in()) :
	echo apply_filters('woocommerce_checkout_must_be_logged_in_message', __('You must be logged in to checkout.', 'theretailer'));
	return;
endif;
